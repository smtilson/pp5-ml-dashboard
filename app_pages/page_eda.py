...
def page_eda_body():
    ...
    axes[0].set_title(f"\n{disp(home_feat)home_feat} p-value = {home_score}")
    sns.histplot(data=data, x=away_feat, kde=True, hue="home_wins", ax=axes[1])
    plt.title(f"{disp(feature_dist)} Distribution: Home vs. Away\n"\
              f"{disp(away_feat)} p-value = {away_score}")
    st.pyplot(fig)
    st.write("In order for the test to determine that the distribution is "\
             "normal, we must have a p-value larger than 0.05.")
    st.write("Notice that there is a certain symmetry when interchanging the "\
            "roles home and away.")
    
    st.write('## Correlation (Spearman) and Predictive Power score\n')
    st.write("Correlation coefficients are measures of the strength of the "\
             "relationship between two random variables. It is a very widely "\
             "used measurement.")
    st.write("The Predictive Power score is an asymmetric statistic that "\
             "can be used to analyze data. It can often find patterns that "\
             "correlation coefficients miss.")
    st.write("Select two features to compare. they will be plotted against "\
    ...
    plt.title(f"{disp(feature_1)} vs. {disp(feature_2)}: "\
              f"{corr_df[feature_1][feature_2]}")
    st.pyplot(fig)
    ppscore = pps_results.query('x == @feature_1 & y == @feature_2')
    ppscore = round(ppscore['ppscore'].iloc[0],4)
    st.write("The respective Predictive Power scores are: \n"\
             f"* {disp(feature_1)} influences {disp(feature_2)}: {ppscore}\n ")
    
    st.write("### Some interesting relationships:")
    st.write("* Year and 3 point shots (made and attempted)")
    st.write("* Aside from point related statistics, the features that had "\
             "the highest correlation with home_wins are: \n"\
             "  * ast_away\n  * ast_home\n  * dreb_away\n  * dreb_home\n"\
             "  * fgm_away\n  * fgm_home")
    st.write("* Only Plus/Minus Home and Pts Home have non-trivial predictive"\
